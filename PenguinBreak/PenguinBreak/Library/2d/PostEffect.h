...
public:
	...
	void Draw(ID3D12GraphicsCommandList* cmdList,const Vec4 &color = { 1.0f,1.0f,1.0f,1.0f }, float time = 0.0f, bool isAlive = true);
	/// <summary>
	/// シーン描画前処理
	/// </summary>
	...
	void UpdatePost(SpriteData& sprite, const Vec2 &position, float width, float height, const Vec2 &anchorpoint, const Vec4 &color, bool isFlipX, bool isFlipY, float time, bool isAlive);
	/// <summary>
	/// ポストエフェクト描画
	/// </summary>
	...
	void DrawPost(SpriteData& sprite, const Vec2 &position, float width, float height, const Vec2 &anchorpoint, const Vec4 &color, bool isFlipX, bool isFlipY, float time, bool isAlive);
	//パイプライン切り替え
