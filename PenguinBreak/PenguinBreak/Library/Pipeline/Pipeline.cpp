...
Pipeline::PipelineSet Pipeline::DoubleTexturePipeline;
...
Pipeline::PipelineSet Pipeline::SpriteCreateDoubleGraphicsPipeline(ID3D12Device* dev, const Shader& shader)
{
	HRESULT result;
	ComPtr<ID3DBlob> errorBlob; // エラーオブジェクト
	if (shader.vsBlob == nullptr)
...
#pragma region//ルートシグネチャの生成
	//デスクリプタテーブルの設定
	CD3DX12_DESCRIPTOR_RANGE  descRangeSRV[2] = {};//定数用
	descRangeSRV[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);//t0レジスタ
	descRangeSRV[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);//t1レジスタ
	//ルートパラメータの設定
